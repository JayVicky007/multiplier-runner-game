...
from sprites import MathGate, Obstacle
...
class LevelManager:
    ...
    def spawn_pair(self) -> None:
        lane_width = SCREEN_WIDTH // 2 - 12
        gate_height = 64
        y = -gate_height - 4
        left_rect = pygame.Rect(16, y, lane_width, gate_height)
        right_rect = pygame.Rect(SCREEN_WIDTH // 2 + 8, y, lane_width, gate_height)
        self.gates.extend(
            (
                MathGate(left_rect, "add", 5, self.font),
                MathGate(right_rect, "add", -3, self.font),
            )
        )
    ...
    def spawn_obstacle(self) -> None:
        ...
        x = 16 + lane * (SCREEN_WIDTH // 2 - 8)
        y = -34
        self.obstacles.append(Obstacle(pygame.Rect(x, y, lane_width, 34)))
